  ...
  def createDataFrame(sparkSession: SparkSession, logicalPlan: LogicalPlan): DataFrame = {
      ...
      for (method <- methods) {
        try {
          ...
          if (paramCount == 2) {
            ...
            return method.invoke(companionInstance, sparkSession, logicalPlan).asInstanceOf[DataFrame]
          } else if (paramCount == 3) {
            // Try (SparkSession, LogicalPlan, Encoder)
            return method.invoke(companionInstance, sparkSession, logicalPlan, encoder).asInstanceOf[DataFrame]
          }
        } catch {
      ...
      for (method <- methods) {
        try {
          ...
          if (paramCount == 0) {
            ...
            return method.invoke(qe, encoder, sparkSession).asInstanceOf[DataFrame]
          }
        } catch {
          case _: Exception => // Try next method
        }
    ...
    try {
      val internalCreateMethod = sparkSession.getClass.getMethods.find { m =>
        m.getName.contains("internalCreate") || m.getName.contains("createDataFrame")
      }
      ...
      if (internalCreateMethod.isDefined) {
        ...
        if (method.getParameterCount == 2) {
          return method.invoke(sparkSession, qe.toRdd, schema).asInstanceOf[DataFrame]
        }
      }
    } catch {
