...
abstract class AbstractTypedColumn[T, U](
  ...
  def /(
    ...
    typed(Divide(self.expr, Literal.create(u, uencoder.catalystRepr)))(
  ...
  def desc(
    ...
    new SortedTypedColumn[T, U](
      SortOrder(expr, Descending, NullsLast, Seq.empty)
    )(uencoder)
  }
  ...
  def asc(
    ...
    new SortedTypedColumn[T, U](
      SortOrder(expr, Ascending, NullsFirst, Seq.empty)
    )(uencoder)
  }
  ...
  def substr[TT1, TT2, W1, W2](
      ...
      w2: frameless.With.Aux[W1, TT2, W2]
    ): ThisType[W2, String] = {
    // Use Catalyst Substring expression directly to avoid Spark 4.0 UnresolvedFunction issue
    import org.apache.spark.sql.catalyst.expressions.Substring
  ...
  def >=(
    ...
    typed(
      GreaterThanOrEqual(self.expr, Literal.create(u, uencoder.catalystRepr))
    )
  ...
  def isin(
    ...
    typed(
      In(self.expr, values.map(v => Literal.create(v, uencoder.catalystRepr)))
    )
  ...
  def between(
    ...
    typed(
      And(
        GreaterThanOrEqual(
          self.expr,
          Literal.create(lowerBound, uencoder.catalystRepr)
        ),
        LessThanOrEqual(
          self.expr,
          Literal.create(upperBound, uencoder.catalystRepr)
        )
      )
    )
  }
