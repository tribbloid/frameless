...
case class TypedRow[L <: HList](
  ...
  def enableOrdering(
      implicit
      ev: MapValues[Caps.AffectOrdering.Enable.type, L]
    ): TypedRow[ev.Out] = {
...
object TypedRow extends RecordArgs {
  ...
  trait Poly extends Poly1 {

    type =>>[T, R] = Case.Aux[T, R]
  }

  case class For[R <: HList]() {

    object NativeOrdering {

      trait Select_Imp0 extends Poly {

        implicit def ignore[T]: T =>> Unit = at[T] { v: T => () }
      }

      object Select extends Select_Imp0 {
    ...
    case class NativeOrdering[MO <: HList](
      )(implicit
        mapper: Mapper.Aux[NativeOrdering.Select.type, R, MO]) {
