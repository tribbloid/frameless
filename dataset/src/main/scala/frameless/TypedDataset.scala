...
class TypedDataset[T] protected[frameless] (
  ...
  private def disambiguate(join: Join): Join = {
    ...
    // Resolve our disambiguation markers directly against the (possibly deduplicated) join's children.
    // This avoids forcing analysis via Dataset/DataFrame construction, which is incompatible in Spark 4.0.
    val leftOut = deduplicatedJoin.left.output
    val rightOut = deduplicatedJoin.right.output

    val newCond = deduplicatedJoin.condition.map(_.transform {
      case FramelessInternals.DisambiguateLeft(tagged: AttributeReference) =>
        leftOut
          .find(_.name == tagged.name)
          .getOrElse(
            throw new RuntimeException(
              s"Attribute ${tagged.name} not found in left plan output"
            )
          )
      ...
      case FramelessInternals.DisambiguateRight(tagged: AttributeReference) =>
        rightOut
          ...
          .getOrElse(
            throw new RuntimeException(
              s"Attribute ${tagged.name} not found in right plan output"
            )
          )
    ...
    deduplicatedJoin.copy(condition = newCond)
  }
