...
object TestReportPlugin extends AutoPlugin {
  ...
  private def parseTestResultXml(xmlFile: File): Option[TestResult] = {
    try {
      ...
      val allTestCases = (xml \ "testcase").flatMap { tc =>
        ...
        if (name.contains("It is not a test it is a sbt.testing.SuiteSelector")) {
          None
        } else {
          val testTime =
  ...
  private def detectSystemTimezone(): java.time.ZoneId = {
    ...
    // 2. user.timezone JVM system property
    // 3. Java's systemDefault() (reads from OS configuration)
    
    sys.env.get("TZ")
      .orElse(sys.props.get("user.timezone"))
      .flatMap { tz =>
        try {
  ...
  private def collectEnvironmentInfo(): Map[String, String] = {
    ...
    Map(
      ...
      "Scala Version" -> props.getOrElse("scala.version", scala.util.Properties.versionNumberString),
      ...
      "user.timezone (JVM Property)" -> props.getOrElse("user.timezone", "not set"),
  ...
  private def generateHtmlReport(
    ...
    val generationTimeUTC = java.time.ZonedDateTime.now(java.time.ZoneId.of("UTC"))
    val generationTimeLocal = generationTimeUTC.withZoneSameInstant(systemTimezone)
    
    // Format timestamps for display
    val dateTimeFormatter = java.time.format.DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss z")
